...
async function finalizeNomination(transaction)
{
    ...
    else if (load.BLQuantity > nom.maxQuantity)
    {
        freightInvoice = load.BLQuantity * nom.wscFlat * nom.wscPercent
            ...
            * nom.wscPercent * nom.overageRate;
    }
    else
    {
        freightInvoice = load.BLQuantity * nom.wscFlat * nom.wscPercent;
    ...
    let freightCommission = freightInvoice * nom.freightCommission;
...
async function initializeStuff(initializing)
{
    ...
    const towageCompany = factory.newResource(NS, 'TowageCompany', 'TowComp-' + random);
    const shippingCompany = factory.newResource(NS, 'ShippingCompany', 'ShipComp-' + random);
    ...
    //loading
    const loading = factory.newResource(NS, 'Loading', 'Load_00' + random);
    loading.nomination = nomination;
    const twoday1 = initializing.timestamp;
    twoday1.setDate(twoday1.getDate() + 2);
    loading.NORTendered = initializing.timestamp;
    loading.documentsOnBoard = twoday1;
    loading.BLQuantity = 138559;

    //discharge
    const tenday = initializing.timestamp;
    tenday.setDate(tenday.getDate() + 10);
    const discharge = factory.newResource(NS, 'Discharge', 'Discharge_00' + random);
